...
if(isset($_POST['chartSubmit'])){
	...
	if(FormValidation::validate($rules)){
		...
		$dob = $y . '-' . $m . '-' . $d;
		$locationCode = $astrologyPlugin->getLocationCode($_POst['birthTown'], $_POST['birthCountry'], $_POST['birthState']);
...
<form action="<?php echo $currentURL; ?>" method="post">
	...
	<fieldset>
		...
		<dd>
			<input type="text" name="birthTown" value="" placeholder="Town/City" required id="personBirthTown">
		</dd>
		...
		<dd>
			<select name="birthState" required id="personBirthState">
